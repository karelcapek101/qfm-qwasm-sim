...
class ChainSimulator:
    def __init__(self):
        ...
        self.global_trace = []
        self.gov = GovernanceInvariants()  # New: Integrate CMM invariants
    ...
    def run_with_gov(self, op_name):
        """New: Run op with governance check"""
        # Apply op (stub for demo)
        output = QFM_Simulator().apply_op(op_name)  # Mock sim
        
        # Mock metrics from state (in real: from QFM state)
        metrics = {
            'topological': np.random.uniform(0.5, 1.5),
            'metabolic': np.linalg.norm(np.random.randn(32)) * 10,  # ~ norm *10
            'dynamical': np.var(np.abs(np.random.randn(32) + 1j * np.random.randn(32)))  # var
        }
        
        invariants = self.gov.evaluate(metrics)
        
        print(f"Step: {op_name}")
        print(f"Output: {output}")
        print(f"Metrics: {metrics}")
        print(f"Invariants: {invariants}")
        if self.gov.responses:
            print(f"Responses: {self.gov.responses}")
        
        # Audit JSON
        audit = self.gov.get_audit()
        print("\nAudit JSON:")
        print(json.dumps(audit, indent=2, default=str))

# Governance Invariants class (new for iter 6)
class GovernanceInvariants:
    def __init__(self):
        self.inv_types = {
            'topological': {'bound': 1.0, 'current': 0.0},
            'metabolic': {'bound': 100.0, 'current': 0.0},
            'dynamical': {'bound': 0.5, 'current': 0.0}
        }
        self.responses = {}
    
    def evaluate(self, metrics):
        results = {}
        for inv, data in self.inv_types.items():
            violation = metrics.get(inv, 0.0) > data['bound']
            data['current'] = metrics.get(inv, 0.0)
            results[inv] = {'status': 'admissible' if not violation else 'violated', 'value': data['current']}
            if violation:
                self._trigger_response(inv)
        return results
    
    def _trigger_response(self, inv_type):
        if inv_type == 'metabolic':
            self.responses[inv_type] = 'throttle (reduce cycles)'
        elif inv_type == 'dynamical':
            self.responses[inv_type] = 'cooldown (pause 10s)'
        else:
            self.responses[inv_type] = 'kill (terminate region)'
    
    def get_audit(self):
        return {
            'invariants': self.inv_types,
            'responses': self.responses
        }

# === DEMO ===
cs = ChainSimulator()
cs.run_with_gov('hamiltonian alpha=1.0 beta=0.5')
cs.run_with_gov('shift p=2')
