...
import json
import hashlib  # Pro SHA256 commitment
...
class ChainSimulator:
    def __init__(self):
        self.chains = {
            ...
            'Kaspa': {'role': 'temporal', 'state': {}, 'traces': []}  # New
        }
        self.global_trace = []
        self.kaspa = KaspaSimulator()  # Integrate Kaspa
    ...
    def execute_on_chain(self, chain, instruction):
        ...
        if chain == 'NEAR':
            ...
            # Anchor commitment from trace
            if 'trace' in instruction:
                trace_id = instruction['trace']['id']
                commitment = hashlib.sha256(str(instruction).encode()).hexdigest()[:8]  # Mock H(TraceID)
...
# Kaspa Simulator (new for iter 7)
class KaspaSimulator:
    def __init__(self):
        self.dag = {}  # {parent: [children]}
        self.roots = []  # Roots
        self.anchors = {}  # {trace_id: hash}
...
# === DEMO ===
cs = ChainSimulator()
qwasm_code = "hamiltonian alpha=1.0 beta=0.5"

print("Output:", cs.execute_on_chain('NEAR', qwasm_code))
print("\nICP Quorum:", cs.execute_on_chain('ICP', "quorum_check"))
trace_id = "trace_123"
print("\nCelestia Store:", cs.execute_on_chain('Celestia', {"trace": {"id": trace_id}}))
print("\nKaspa Anchor:", cs.execute_on_chain('Kaspa', {"trace": {"id": trace_id}}))
print("\nReplay Verify:")
for res in cs.replay()[-4:]:  # Last 4
    print(res)
